...
import { Loader2, MapPin, Phone, Clock, Navigation, Mail } from "lucide-react";
      ...
      <div className="container mx-auto px-4 py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {locations?.map((location) => {
            ...
            return (
              <div key={location.id} className="bg-card border border-white/5 rounded-xl overflow-hidden shadow-lg group hover:border-primary/50 transition-colors">
                ...
                <div className="p-8">
                  <h2 className="text-3xl font-display font-bold uppercase mb-4 text-white">{location.name}</h2>
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    {/* Add Email Block if exists */}
                    {(location as any).email && (
                      <div className="flex items-center gap-3 text-neutral-400">
                        <Mail className="w-5 h-5 shrink-0 text-primary" />
                        <p>{(location as any).email}</p>
                      </div>
                    )}
                    <div className="flex items-start gap-3 text-neutral-400">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button variant="outline" className="w-full font-bold uppercase tracking-wider text-white border-white/20 hover:bg-white/10 hover:text-white">
                      Call Now
                    </Button>
